...
void debug_print_cache(struct cache *c, char *cache_type)
{
    fprintf(debug_file, "%s %s cache: Total number of entries: %d\n", c->name, cache_type, c->number_of_cache_entries);
    fprintf(debug_file, "%s %s cache: %s\n", c->name,  cache_type, print_sets_and_ways(c));
    fprintf(debug_file, "%s %s cache: Each cache line is %d bytes\n", c->name,  cache_type, c->cache_line_size);
    fprintf(debug_file, "%s %s cache: Cache is %s\n", c->name,  cache_type, c->write_back ? "write-back" : "write-thru");
    fprintf(debug_file, "%s %s cache: With a %s replacement policy\n", c->name, cache_type, CRP_name(c));
}
...
void debug_print_cds(CDS *cds)
{
	...
	if (cds->vc != NULL) {
		debug_print_cache(cds->vc, "victim");
	}
...
void Print_Cache_Statistics_for_one_cds(CDS *cds)
{
    ...
    if (cds->vc != NULL) {
    	Print_Cache_Statistics_for_one_cache(cds->vc);
    }
...
void init_cache(CDS *cds)
{
    ...
    if (cds->vc != NULL) {
    	cds->vc->c_line = CAST(cache_line *, calloc(cds->vc->number_of_cache_entries, sizeof(cache_line)));
    }
}
