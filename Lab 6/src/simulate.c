...
{
    int cache_entry_index = Compute_Set_Index(c, cache_address);		// at block 4
...
Boolean evict_from_cache(CDS *cds, cache_line *victim_line, memory_address cache_address)
{
    ...
    if (victim_line->dirty)
        {
            evict_dirty_line_from_cache(cds->c, victim_line);		// at block 6
        }
...
/* ***************************************************************** */
	...
	// 1) search the base cache
	// 2) if not there, select a victim
	// 3) search the victim cache (if applicable)
	// 		a) if present, swap the victim cache line with the main cache victim; done
	//		b) else not present; select a victim from the victim cache
	// 4) evict the victim from the victim cache
	// 5) put the victim from the main cache into the victim cache
	// see piazza post #1089

void Simulate_Reference_to_Cache_Line(CDS *cds, memory_reference *reference)
{
    Boolean Found = FALSE;
    cache_line *cache_entry = NULL;
    Boolean vc_found = FALSE;
    cache_line *victim_cache_entry = NULL;
    int victim_cache_entry_index;
    ...
    memory_address cache_address = Base_Cache_Address(cds->c, reference->address);		// at block 4; no modification needed
    
    // cache_address gives the address for the start of the cache line

    int cache_entry_index = Search_Cache_For(cds->c, cache_address);		// at block 5; no modification needed
    if (cache_entry_index >= 0) {
    	...
    	if (cache_entry->valid) {
    		if (cds->vc != NULL) {
    			...
    			victim_cache_entry_index = Search_Cache_For(cds->vc, cache_address);
    			cds->vc->number_total_cache_access++;

    			if (victim_cache_entry_index >= 0) {
    				vc_found = TRUE;
    				victim_cache_entry = &(cds->vc->c_line[victim_cache_entry_index]);
    				swap_cache_lines(cache_entry, victim_cache_entry);
    			}
    			else {
    				victim_cache_entry_index = Find_Victim_by_Replacement_Policy(cds->vc, cache_address);
    				victim_cache_entry = &(cds->vc->c_line[victim_cache_entry_index]);
    				cds->vc->number_miss_reads++;

    				if (victim_cache_entry->dirty) {
    					evict_dirty_line_from_cache(cds->vc, victim_cache_entry);
    				}
    			...
    			Set_Replacement_Policy_Data(cds->number_of_memory_reference, cds->vc, victim_cache_entry);
    		}
    		else {
    ...
    if (reference->type == MAT_STORE) {
    	// if it's not write-back, then it's write-through; for write-through, if it's a write, write to memory
    	if (!cds->c->write_back) {
    		cds->c->number_miss_writes++;
...
void Simulate_Reference_to_Memory(CDS *cds, memory_reference *reference)
{
    ...
    if (Base_Cache_Address(cds->c, reference->address) ==								// at block 4; no modification needed
    		Base_Cache_Address(cds->c, reference->address + reference->length -1))
        {
            Simulate_Reference_to_Cache_Line(cds, reference);							// at block 8
        }
    else
        {
            ...
            Simulate_Reference_to_Cache_Line(cds, &reference1);		// at block 8
            Simulate_Reference_to_Cache_Line(cds, &reference2);		// at block 8
        }
}
...
void Simulate_Caches(String trace_file_name)
{
    ...
    Init_caches_for_trace();	// in cds.c; no modification needed

    while (Read_trace_file_line(trace_file, &reference) != EOF)		// at block 2; no modification needed
        {
            CDS *cds = CDS_root;
            while (cds != NULL)
                {
                    Simulate_Reference_to_Memory(cds, &reference);	// at block 9
                    cds = cds->next;
                }
        }
    fclose(trace_file);
