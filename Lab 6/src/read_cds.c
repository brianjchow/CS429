...
void define_key_value_pair(CDS *cds, Token *key, Token *value)
{
    ...
    if (strcasestr((*key).string, "victim") != NULL) {
    	...
    	// if main cache is write-back, then victim cache is write-back as well
    	// else if main cache is write-through, then it doesn't matter for victim cache

    	cds->vc = CAST(struct cache *, calloc(1, sizeof(struct cache)));

    	int victim_cache_num_entries = atoi((*value).string);

    	cds->vc->cache_line_size = cds->c->cache_line_size;					// assumes cache line size has already been read
    	cds->vc->number_of_cache_entries = victim_cache_num_entries;
    	cds->vc->number_of_ways = victim_cache_num_entries;
    	cds->vc->write_back = cds->c->write_back;
    	cds->vc->replacement_policy = CRP_FIFO;
    	cds->vc->LFU_Decay_Interval = 0;									// not necessary
    	cds->vc->c_line = NULL;
...
CDS *Read_CDS_file_entry(FILE *CDS_file)
{
    ...
    // change names so printout will reflect both caches
    if (cds->vc != NULL) {
    	cds->c->name = augment_name(cds->name, "main cache");
    	cds->vc->name = augment_name(cds->name, "victim cache");
    }
    else {
...
//    victim_cache_specified = FALSE;
//    victim_cache_num_entries = 0;
